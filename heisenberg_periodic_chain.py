...
print("\nProgram for calculating ground state and first excited state energy periodic Heisenberg spin half chain for N-sites\n")
import math
import numpy as np
...
J = int(input("Enter value of exchange coupling, J : "))
...
for i in range(2**N):
    for j in range(N):
        if j!=N-1:
            ...
            result_bases[i] += SzSz_p(chains[i],j) + 0.5*(SpSm_p(chains[i],j) + SmSp_p(chains[i],j))
...
# writing data
file = open("heisenberg_periodic_chain_output.txt", "w")
file.write("Periodic chain\n")
file.write("Number of sites : " +str(N) + "\n")
file.write("Value of J : "+str(J) +"\n")
